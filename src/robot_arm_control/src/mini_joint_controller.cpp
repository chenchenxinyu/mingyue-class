...
void posCmdCB(const std_msgs::Float64& pos_cmd_msg){
    ROS_INFO("received value of pos_cmd_msg is %f",pos_cmd_msg.data);
...
bool test_services(){
    ...
    {
        /* code */
        ROS_WARN("waitng for apply_joint_effort service");
        return false;
    }
    if (!ros::service::exists("/gazebo/get_joint_properties",true))
    {
        ROS_WARN("waiting for get_joint_properties service");
        return false;
    }
...
int main(int argc, char **argv)
{
    ...
    ros::NodeHandle nh;
    ros::Duration half_sec(0.5);//a nap
